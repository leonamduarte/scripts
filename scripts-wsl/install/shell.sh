...
if ! has_command fish; then
	if sudo apt-add-repository -y ppa:fish-shell/release-3 2>/dev/null; then
		if sudo apt-get update; then
			...
			log_error "Falha ao atualizar repositórios apt"
			exit 1
		fi
	else
		log_error "Falha ao adicionar PPA do Fish"
		exit 1
	fi
else
...
if [[ "$SHELL" != *"fish"* ]]; then
	...
	if [[ -n "$FISH_PATH" ]]; then
		if chsh -s "$FISH_PATH"; then
			...
			log_warn "Não foi possível alterar o shell padrão (pode precisar de permissão)"
		fi
	else
		log_warn "Caminho do Fish não encontrado, pulando configuração do shell padrão"
	fi
fi
