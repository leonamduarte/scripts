...
dnf install -y --skip-unavailable \
  @development-tools git curl wget unzip tar jq fzf ripgrep fd-find \
  ...
  flatpak fwupd \
  kooha stow\
  kitty wezterm alacritty vlc fira-code-fonts jetbrains-mono-fonts-all\
...
if ! command -v flatpak >/dev/null 2>&1; then
  dnf install -y --skip-unavailable flatpak >>"$LOG" 2>&1
fi
