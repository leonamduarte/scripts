...
<#
.SYNOPSIS
    Bootstrap WSL - Instalador de Fedora com TUI
.DESCRIPTION
    Script PowerShell com interface interativa para instalar e verificar WSL/Fedora
.EXAMPLE
    .\bootstrap.ps1
#>
...
function Show-Header {
    ...
    Write-Host "║           🐧 WSL Bootstrap - Fedora Installer            ║" -ForegroundColor $Colors.Primary
    Write-Host "╚══════════════════════════════════════════════════════════╝" -ForegroundColor $Colors.Primary
    Write-Host ""
}
...
function Show-Menu {
    ...
    Write-Host ""
    Write-Host "    [1] 📦 Instalar Fedora Linux no WSL" -ForegroundColor $Colors.Info
...
function Get-LatestFedoraDistro {
    try {
        $online = wsl --list --online 2>$null
        $matches = @()
        foreach ($line in $online) {
            if ($line -match 'FedoraLinux-\d+') {
                $matches += [regex]::Match($line, 'FedoraLinux-\d+').Value
            }
        }
        if ($matches.Count -gt 0) {
            return ($matches | Sort-Object {[version]($_ -replace '[^\d.]', '')} | Select-Object -Last 1)
        }
    } catch {}
    return "FedoraLinux"
}
...
function Install-Fedora {
    $distro = Get-LatestFedoraDistro
    Show-Header
    Write-Host "  📦 Instalando $distro..." -ForegroundColor $Colors.Primary
    Write-Host ""
    
    # Verificar se já existe Fedora
    Show-Progress "Verificando instalações existentes..."
    try {
        $wslList = wsl --list --quiet 2>$null
        if ($wslList -match "Fedora") {
            Show-Success "Verificação concluída"
            Write-Host ""
            Show-Warning "Fedora já está instalado!"
            Write-Host ""
    ...
    # Instalar Fedora
    Write-Host ""
    Show-Progress "Iniciando instalação do $distro..."
    Write-Host ""
    
    try {
        # Executar instalação com output visível
        $process = Start-Process -FilePath "wsl.exe" -ArgumentList "--install", "-d", $distro -Wait -PassThru -NoNewWindow
        
        if ($process.ExitCode -eq 0) {
            Show-Success "$distro instalado com sucesso!"
            Write-Host ""
        ...
        Write-Host ""
        Show-Info "Tente executar manualmente: wsl --install -d $distro"
    }
