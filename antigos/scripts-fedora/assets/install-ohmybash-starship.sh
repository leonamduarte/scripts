...
main() {
    ...
    if [ -d "$HOME/.oh-my-bash" ]; then
        ok "Oh My Bash ja esta instalado."
    else
    ...
    if command -v starship >/dev/null 2>&1; then
        ...
        # Tenta instalar via DNF primeiro
        if sudo dnf install -y starship >> "$LOG_FILE" 2>&1; then
            ok "Starship instalado via DNF."
        else
            # Fallback: COPR
            warn "Starship nao encontrado nos repos oficiais, tentando COPR..."
            if ensure_copr_package "atim/starship" "starship"; then
                ok "Starship instalado via COPR."
            else
                warn "Tentando via script oficial..."
                curl -sS https://starship.rs/install.sh | sh -s -- -y || warn "Falha ao instalar Starship via script."
            fi
        fi
    fi

    # --- Configuracao no Bash -------------------------------------------------
    if ! grep -q 'eval "$(starship init bash)"' "$HOME/.bashrc"; then
        info "Configurando Starship no Bash..."
        echo '' >> "$HOME/.bashrc"
        echo '# Inicializacao do Starship prompt' >> "$HOME/.bashrc"
        echo 'eval "$(starship init bash)"' >> "$HOME/.bashrc"
        ok "Starship configurado no Bash."
    else
        ok "Starship ja configurado no Bash."
    fi
