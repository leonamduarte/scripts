...
# Estrategia: Systemd Automount (On-demand)
...
require_cmd() {
    if ! command -v "$1" >/dev/null 2>&1; then
        die "Comando requerido nao encontrado: $1"
    fi
}
...
if [[ "$REAL_USER" == "root" ]]; then
    warn "Executando como root puro. Symlinks serao criados em /root/."
fi
...
info "Configurando automount para usuario: $REAL_USER ($REAL_UID)"
...
[[ -b "$DEV_DEV" ]] || die "Particao 'dev' nao encontrada (LABEL=${LABEL_DEV} ou ${DEV_DEV_CAND})."
...
[[ -b "$DEV_1TB" ]] || die "Particao '1TB' nao encontrada (LABEL=${LABEL_1TB} ou ${DEV_1TB_CAND})."
...
if ! mount -a >> "$LOG_FILE" 2>&1; then
    ...
    cp -af "$BACKUP" "$FSTAB"
    systemctl daemon-reload
    die "Erro critico ao montar. Rollback aplicado com sucesso."
fi
...
ok "Configuracao de disco concluida. Acesso em ~/dev e ~/1TB"
